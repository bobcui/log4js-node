...
function loadConfigurationFileAsync(filename, callback) {
    fs.readFile(filename, 'utf8', function(err, data) {
        if (!!err) {
            console.error('read log4js config file %s fail err=%s', filename, err.stack)
            callback(null)
        }
        else {
            var config
            try {
                ...
                console.log('reload log4js config file %s mtime=%s', filename, configState.lastMTime.getTime())
                console.log(config)
            }
            catch (e) {
                console.error('reload log4js config file %s failed mtime=%s err=%s', filename, configState.lastMTime.getTime(), e.stack)
            }
            callback(config)
        }
    })
...
function getMTimeAsync(filename, callback) {
    fs.stat(filename, function(err, stats){
        if (!!err) {
            console.error('stat log4js config file %s fail err=%j', filename, err.stack)
            callback(null)
        }
        else {
...
function initReloadConfiguration(filename, options) {
  ...
  configState.timerId = setInterval(reloadConfigurationAsync, options.reloadSecs*1000);
}
...
module.exports = {
  ...
  configureOnceOff: configureOnceOff
};
